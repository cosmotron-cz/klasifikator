...
class TestElasticHandler(unittest.TestCase):
    ...
    def test_save_konspekt(self):
        ...
        try:
            ...
            response = ElasticHandler.save_document(index, document)
            self.assertEqual('created', response['result'])
            konspekt = [{'category': "category1", 'group': "group1", 'description': "description1"},
                        {'category': "category2", 'group': "group2", 'description': "description2"}]
    ...
    def test_save_keywords(self):
        ...
        try:
            ...
            response = ElasticHandler.save_keywords(index, response['_id'], keywords)
            self.assertEqual('updated', response['result'])
        finally:
            ElasticHandler.remove_index(index)
